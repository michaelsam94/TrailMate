...
object GraphPathFinder {
    ...
    fun aStar(
        ...
        while (open.isNotEmpty()) {
            ...
            for (neighbor in graph.adjacencyList[current].orEmpty()) {
                if (neighbor in options.bannedNodes && neighbor != target) continue
                val step = edgeCost(graph, current, neighbor, options) ?: continue
    ...
    fun dijkstraAll(
        ...
        while (queue.isNotEmpty()) {
            ...
            for (v in graph.adjacencyList[u].orEmpty()) {
                if (v in options.bannedNodes && v != source) continue
                val step = edgeCost(graph, u, v, options) ?: continue
    ...
    fun pathCost(
        ...
        for (i in 0 until nodeIds.size - 1) {
            total += edgeCost(graph, nodeIds[i], nodeIds[i + 1], options) ?: return Double.MAX_VALUE
        }
        return total
    }
    ...
    private fun edgeCost(
        ...
        if (edge in options.blockedEdges) return null
        val nodeU = graph.nodes[u] ?: return null
        val nodeV = graph.nodes[v] ?: return null
        var cost = haversineKm(nodeU.lat, nodeU.lng, nodeV.lat, nodeV.lng)
        cost *= options.edgePenalties[edge] ?: 1.0
